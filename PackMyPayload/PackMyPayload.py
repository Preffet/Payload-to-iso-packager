...
'''
    ...
    opt.add_argument('-i', '--backdoor', default='', type=str,
                     help='Instead of generating blank new output container/archive, will backdoor existing input one.')
    opt.add_argument('-n', '--filename', default='', metavar='NAME',
                     help='Package input file into archive/container under this filename (may contain relative path).')
    opt.add_argument('-p', '--password', default='', metavar='PASSWORD',
                     help='If output archive/container format supports password protection, use this password to protect output file.')
    opt.add_argument('--out-format', default='', choices=lib.packager.Packager.formatsMap.keys(),
                     help='Explicitely define output format disregarding output file\'s extension. Can be one of following: ' + ', '.join(
                         lib.packager.Packager.formatsMap.keys()))
    ...
    vhdopts.add_argument('--vhd-size', default=1024, type=int, metavar='SIZE',
                         help='VHD dynamic size in MB. Default: 1024')
    vhdopts.add_argument('--vhd-letter', default='', metavar='LETTER',
                         help='Drive letter where to mount VHD drive. Default: will pick unused one at random.')
    vhdopts.add_argument('--vhd-filesystem', default='fat32', choices=['fat', 'fat32', 'ntfs'], metavar='FS',
                         help='Filesystem to be used while formatting VHD. Default: FAT32. Supported: fat, fat32, ntfs')
...
def main(argv):
    ...
    if outputFormat == '':
        ...
        logger.err('Please use on of the following output file\'s extensions: ' + ', '.join(
            lib.packager.Packager.formatsMap.keys()))
        return False
